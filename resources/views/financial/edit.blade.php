...
@section('container')
    <form action="{{ route('financials.update', ['financial' => $financial->id]) }}" method="POST">
        @csrf
        @method('PUT')
            ...
            <div class="row mx-3 bg-white px-2 py-4 justify-content-evenly shadow-lg" style="border-radius: 10px">
                ...
                <div class="col-md-6">
                    ...
                    <input type="text" name="name" class="form-control" id="exampleFormControlInput1"
                        placeholder="Silahkan masukkan name" value="{{ old('name', $financial->name) }}" />
                    @error('name')
                ...
                <div class="col-md-6">
                    ...
                    <input type="date" name="tanggal" class="form-control py-1" id="exampleFormControlInput1"
                        placeholder="Silahkan Masukkan Last Name" value="{{ old('tanggal', $financial->tanggal) }}" />
                    @error('tanggal')
                        <p class="text-danger mt-1 mb-1 " style="font-size: 12px">{{ $message }}</p>
                    @enderror
                </div>
                ...
                <div class="col-md-12">
                    ...
                    <input type="number" name="nominal" class="form-control" id="exampleFormControlInput1"
                        placeholder="Silahkan masukkan Nominal"
                        value="{{ old('nominal', $financial->pemasukan === 0 ? $financial->pengeluaran : $financial->pemasukan) }}" />
                    @error('nominal')
                        <p class="text-danger mt-1 mb-1 " style="font-size: 12px">{{ $message }}</p>
                    @enderror
                </div>
                <div class="col-md-12 mb-5">
                    ...
                    <input type="text" name="detail" class="form-control" id="exampleFormControlInput1"
                        placeholder="Silahkan masukkan Detail" value="{{ old('detail', $financial->detail) }}" />
                    @error('detail')
                        <p class="text-danger mt-1 mb-1 " style="font-size: 12px">{{ $message }}</p>
                    @enderror
                </div>
